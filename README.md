...
This default command starts the standard PyTorch inference path. This branch also includes a fast backend for DM05 that uses a TensorRT vision encoder together with a Triton suffix big-kernel decode path.

Install the fast inference optional dependencies before building or auto-building the TensorRT engine:

```bash
pip install -e ".[fast-infer]"
```

The vision TensorRT engine is a separate artifact. It is not downloaded with a checkpoint, but the fast inference startup path can now auto-build it when needed. To build it manually from the DM05 checkpoint at `./checkpoints/DM05-libero`, run:

```bash
python -m opendm.infer.build_vision_trt \
  --checkpoint ./checkpoints/DM05-libero \
  --onnx-path ./checkpoints/DM05-libero/dm05_vision.onnx \
  --engine-path ./checkpoints/DM05-libero/dm05_vision.engine \
  --num-images 3
```

The example above matches the default inference service image keys `images_1 images_2 images_3`. `--num-images` must equal the number of image keys used by the inference service. If you change `--inference-config.image-keys`, rebuild the engine with the same image count.

Enable the fast backend only when the runtime can load TensorRT and Triton successfully. The launcher will automatically ensure a compatible vision TensorRT engine exists before the service starts:
...
script/dm05_launcher.sh \
  ...
  --inference-config.vision-trt-engine-path ./checkpoints/DM05-libero/dm05_vision.engine \
  --inference-config.port 7891
```

On an RTX 4090, under a non-concurrent LIBERO benchmark setup, the average end-to-end latency is about `115 ms` with the fast backend and about `655 ms` with the default backend.
...
- `--inference-config.backend`: inference backend. Use `default` for the standard path, or `fast` for the TensorRT plus Triton fast path.
- `--inference-config.image-keys`: image keys consumed by the inference service. When `--inference-config.backend fast` is enabled, the number of image keys must equal the TensorRT engine `--num-images` used in `build_vision_trt.py`.
- `--inference-config.vision-trt-engine-path`: optional when `--inference-config.backend fast` is enabled. It points to the TensorRT vision engine used by the fast path. If omitted, the launcher defaults to `./checkpoints/DM05-libero/dm05_vision.engine` and auto-builds it on demand.
- `--inference-config.force-rebuild`: optional flag. When set together with `--inference-config.backend fast`, the launcher rebuilds the TensorRT engine even if a matching engine already exists.
...
- `image`: image files, one field per configured image key. The order must match `--inference-config.image-keys`. With the fast backend, the total number of uploaded images must also match the TensorRT engine image count.
