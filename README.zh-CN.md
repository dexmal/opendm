...
上面的命令会启动默认的 PyTorch 推理路径。这个分支还额外包含一条 DM05 低延迟 fast backend：使用 TensorRT 视觉编码器和 Triton suffix big-kernel decode 路径。

在构建或自动构建 TensorRT engine 之前，请先安装 fast inference 的可选依赖：

```bash
pip install -e ".[fast-infer]"
```

vision TensorRT engine 是单独导出的 artifact，不会随着 checkpoint 下载，但 fast 推理启动路径现在会在需要时自动构建。若要基于 `./checkpoints/DM05-libero` 下的 DM05 checkpoint 手动预构建，可执行：
...
上面的示例对应默认推理服务的图像键 `images_1 images_2 images_3`。`--num-images` 必须等于推理服务使用的 image key 数量；如果修改了 `--inference-config.image-keys`，就需要用相同的图片数量重新构建 engine。

只有在运行环境里可以正常加载 TensorRT 和 Triton 时，才建议启用 fast backend。启动脚本会在服务启动前自动确保兼容的 vision TensorRT engine 已存在：
...
在 RTX 4090 上、无并发的 LIBERO benchmark 条件下，平均端到端延迟约为 `115 ms`（fast backend）和 `655 ms`（default backend）。
...
- `--inference-config.backend`：推理后端。`default` 表示标准路径，`fast` 表示 TensorRT 加 Triton 的低延迟路径。
- `--inference-config.image-keys`：推理服务使用的图像键。启用 `--inference-config.backend fast` 时，其数量必须等于 `build_vision_trt.py` 中 TensorRT engine 的 `--num-images`。
- `--inference-config.vision-trt-engine-path`：当 `--inference-config.backend fast` 启用时可选，指向 fast path 使用的 TensorRT vision engine。如果不传，启动脚本会默认使用 `./checkpoints/DM05-libero/dm05_vision.engine`，并在缺失时自动构建。
- `--inference-config.force-rebuild`：可选参数。与 `--inference-config.backend fast` 一起使用时，即使已有匹配的 engine 也会强制重建。
...
- `image`：图像文件，每个配置的图像键对应一个 `image` 字段，顺序需要与 `--inference-config.image-keys` 一致。启用 fast backend 时，上传的图片总数还必须与 TensorRT engine 的图片数量一致。
